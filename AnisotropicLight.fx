...
PS_OUTPUT PerPixelPS(VS_OUTPUT In)
{
	...
	float u = dot(light, norm);
	float v = dot(h, norm);
	
	float4 anisodiff = tex2D(anisomap, float2(u/2 + 0.5,v/2+0.5));
	float3 diff = anisodiff.rgb;
	float spec = anisodiff.a*0.7;
	...
	float3 totalSpecular = g_pMatSpecular*g_pLightSpecular.xyz * spec;
